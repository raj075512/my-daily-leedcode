...
int solve(Node * l,Node * r, int target)
{
    if(!r||!l) return -1;
    if(r->data==target) return l->data;
    if(l->data==target) return r->data;
    
   
  int ans= solve(l->left,r->right,target);
  if(ans!=-1) return ans;
  else
  return solve(l->right,r->left,target);
